    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        if cfg.stage == 'param':
            ...
            joint_coord_cam = torch.bmm(
                torch.from_numpy(self.joint_regressor).cuda()[None, :, :].repeat(mesh_coord_cam.shape[0], 1, 1),
                mesh_coord_cam)
        ...
        if mode == 'train':
            ...
            loss = {}
            if cfg.stage == 'lixel':
                loss['joint_fit'] = self.coord_loss(joint_coord_img, targets['fit_joint_img'],
                                                    meta_info['fit_joint_trunc'] * meta_info['is_valid_fit'][:, None,
                                                                                   None])
                loss['joint_orig'] = self.coord_loss(joint_coord_img, targets['orig_joint_img'],
                                                     meta_info['orig_joint_trunc'], meta_info['is_3D'])
                loss['mesh_fit'] = self.coord_loss(mesh_coord_img, targets['fit_mesh_img'],
                                                   meta_info['fit_mesh_trunc'] * meta_info['is_valid_fit'][:, None,
                                                                                 None])
                loss['mesh_joint_orig'] = self.coord_loss(joint_img_from_mesh, targets['orig_joint_img'],
                                                          meta_info['orig_joint_trunc'], meta_info['is_3D'])
                loss['mesh_joint_fit'] = self.coord_loss(joint_img_from_mesh, targets['fit_joint_img'],
                                                         meta_info['fit_joint_trunc'] * meta_info['is_valid_fit'][:,
                                                                                        None, None])
                loss['mesh_normal'] = self.normal_loss(mesh_coord_img, targets['fit_mesh_img'],
                                                       meta_info['fit_mesh_trunc'] * meta_info['is_valid_fit'][:, None,
                                                                                     None]) * cfg.normal_loss_weight
                loss['mesh_edge'] = self.edge_loss(mesh_coord_img, targets['fit_mesh_img'],
                                                   meta_info['fit_mesh_trunc'] * meta_info['is_valid_fit'][:, None,
                                                                                 None])
            else:
                loss['pose_param'] = self.param_loss(pose_param, targets['pose_param'],
                                                     meta_info['is_valid_fit'][:, None])
                loss['shape_param'] = self.param_loss(shape_param, targets['shape_param'],
                                                      meta_info['is_valid_fit'][:, None])
                loss['joint_orig_cam'] = self.coord_loss(joint_coord_cam, targets['orig_joint_cam'],
                                                         meta_info['orig_joint_valid'] * meta_info['is_3D'][:, None,
                                                                                         None])
                loss['joint_fit_cam'] = self.coord_loss(joint_coord_cam, targets['fit_joint_cam'],
